  ...
  module Initiatives
    describe InitiativeForm do
      ...
      let(:attributes) do
        {
          ...
          add_documents: documents_params
      ...
      context "when the attachment is present" do
        ...
        context "when documents are invalid" do
          ...
          it "adds an error to the `:add_documents` field" do
            expect(subject).not_to be_valid
            expect(subject.errors.full_messages).to match_array(["Add documents needs to be reattached", "Add documents one or more files seems to not have the right format, format accepted are PDF"])
            expect(subject.errors.keys).to match_array([:add_documents])
          end
        end
      end
    end
